...
class TagController extends CI_Controller{
    ...
    public function vTag($idTag){
        ...
        $data['tag']        = $this->Tag->getById($idTag);
        $data['idTag']      = $idTag;
    ...
    public function getDataTask($idTag){
        $currDate = date('Y-m-d');
        echo json_encode([
            'HTMLTASK'       => $this->getTask($idTag, $currDate),
            'HTMLCOMPLETEDS' => $this->getCompleteds($idTag)
        ]);
    }
    public function getTask($idTag, $currDate){
        $tasks  = $this->General->get('v_task_tag', ['ID_TAG' => $idTag, 'orderBy' => 'TGL_TASK ASC', 'ISFINISHED_TASK' => '0']);
        $html   = '';
        if($tasks != null){
            foreach ($tasks as $item) {
                $status = "";
                $task = $this->Task->getById($item->ID_TASK);
                if($task->PRIORITAS_TASK != "0"){
                    $status = '
                        <span class="ps-4 '.$task->COLOR_MP.'" style="font-weight: 700;" data-bs-toggle="tooltip" data-bs-placement="top" title="'.$task->NAMA_MP.'">'.$task->ALIAS_MP.'&nbsp;</span>
                    ';
                }

                $date = "";
                if($task->TGL_TASK != null){
                    if($task->TGL_TASK < $currDate){
                        $date='
                            <span class="text-danger">'.date_format(date_create($task->TGL_TASK), 'j M Y H:i').'</span>
                        ';
                    }else{
                        $date='
                            <span class="text-secondary">'.date_format(date_create($task->TGL_TASK), 'j M Y H:i').'</span>
                        ';
                    }
                }
                        ...
                        <div class="menu-item">
                            <div class="menu-link">
                                ...
                                <label class="ps-lg-3 form-check form-check-custom form-check-solid me-10" >
                                    ...
                                    '.$status.'
                                    <span class="fw-bold ps-4">'.$task->NAMA_TASK.'</span>
                                </label>
                                <div style="right: 5px;">
                                    '.$taskTagHtml.'
                                    '.$date.'
    ...
    public function getCompleteds($idTag){
        $completeds = $this->General->get('v_task_tag', ['ID_TAG' => $idTag, 'orderBy' => 'TGL_TASK DESC', 'ISFINISHED_TASK' => '1']);
        $html       = '';
        if($completeds != null){
            foreach ($completeds as $item) {
                $task = $this->Task->getById($item->ID_TASK);
                $status = "";
                if($task->PRIORITAS_TASK){
                    $status = '
                        <span class="ps-4" data-bs-toggle="tooltip" data-bs-placement="top" title="'.$task->NAMA_MP.'">'.$task->ALIAS_MP.'&nbsp;</span>
                    ';
                }
                    ...
                    <div class="menu-item">
                        <div class="menu-link">
                            ...
                            <label class="ps-lg-3 form-check form-check-custom form-check-solid me-10">
                                ...
                                '.$status.'
                                <span class="fw-bold ps-4" style="text-decoration: line-through;">'.$task->NAMA_TASK.'</span>
                            </label>
                            '.$taskTagHtml.'
                        </div>
                    </div>
